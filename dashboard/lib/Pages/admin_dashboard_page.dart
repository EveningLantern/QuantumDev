...
class _AdminDashboardPageState extends State<AdminDashboardPage>
  ...
  // New Hero Section with text animation
  ...
  Future<void> _selectDateRange(BuildContext context) async {
    ...
    if (picked != null) {
      ...
      final int daysDifference = picked.end.difference(picked.start).inDays;

      // If the range is more than 10 days, adjust the end date
      if (daysDifference > 10) {
        setState(() {
          _startDate = picked.start;
          _endDate = picked.start.add(const Duration(days: 10));
        });

        // Show a message to the user
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Date range limited to 10 days maximum for clarity'),
